...
const countDisplayValue = (value = props.value, scale = props.scale) => Math.floor(value / scale).toString()
...
describe('components/BaseInputRange.vue', () => {
  ...
  test('input has auto set width', async () => {
    ...
    test("has min value with suffix displayed", () => base.testPropInline(({ getByText }) => getByText(`${props.min} ${props.suffix}`)))
    test("has max value with suffix displayed", () => base.testPropInline(({ getByText }) => getByText(`${props.max} ${props.suffix}`)))
    test("has name displayed", ()=> base.testPropInline(({getByText})=> getByText(props.name)))

  test("emit change on input of number type", () => base.testEmitter(({ getAllByDisplayValue }) => getAllByDisplayValue(countDisplayValue())[0], "input"))
  test("emit change on input of range type", () => base.testEmitter(({ getAllByDisplayValue }) => getAllByDisplayValue(countDisplayValue())[1], "input"))
})
